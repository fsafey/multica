...
type CancelDeferredEscalationsForIssueAgentRow struct {
	...
	WorkflowAttemptID              pgtype.UUID        `json:"workflow_attempt_id"`
	WorkflowClaimEpoch             pgtype.Int8        `json:"workflow_claim_epoch"`
	WorkflowInputDigest            pgtype.Text        `json:"workflow_input_digest"`
	WorkflowLawDigest              pgtype.Text        `json:"workflow_law_digest"`
...
WHERE id = (
    ...
    WHERE atq.agent_id = $2
      ...
      AND ($3::uuid IS NULL OR atq.runtime_id = $3::uuid)
      AND NOT EXISTS (
...
func (q *Queries) ClaimAgentTask(ctx context.Context, arg ClaimAgentTaskParams) (AgentTaskQueue, error) {
	row := q.db.QueryRow(ctx, claimAgentTask, arg.PrepareLeaseSecs, arg.AgentID, arg.RuntimeID)
	var i AgentTaskQueue
	err := row.Scan(
		&i.ID,
		&i.AgentID,
