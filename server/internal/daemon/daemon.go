...
func (d *Daemon) reportTerminalTask(parentCtx context.Context, report terminalTaskReport) error {
	...
	case terminalTaskReportComplete:
		return d.client.CompleteTask(ctx, report.taskID, report.output, report.branchName, report.sessionID, report.workDir, report.transcriptExpectation, report.sessionRolloutMissing, report.retiredSessionID)
	case terminalTaskReportFail:
		return d.client.FailTask(ctx, report.taskID, report.errorMessage, report.sessionID, report.workDir, report.failureReason, report.transcriptExpectation, report.sessionRolloutMissing, report.retiredSessionID)
	default:
...
func (d *Daemon) runTask(ctx context.Context, task Task, provider string, slot int, taskLog *slog.Logger) (taskResult TaskResult, returnErr error) {
	...
	result, tools, err := d.executeAndDrain(ctx, backend, prompt, execOpts, taskLog, task.ID, env.CodexHome, &msgSeq, &transcriptDelivered)
	if err != nil {
	...
	if shouldRetryWithFreshSession(result, task.PriorSessionID, tools, provider) {
		...
		if retryEvidenceErr != nil {
			...
			// provider transcript is gone, so the fresh process must not be told it
			// is resuming a conversation. These values are task-local.
			...
			retryResult, retryTools, retryErr := d.executeAndDrain(ctx, backend, freshPrompt, execOpts, taskLog, task.ID, env.CodexHome, &msgSeq, &transcriptDelivered)
			if retryErr != nil {
	...
	case "completed":
		...
		taskResult = withTranscriptExpectation(TaskResult{
			Status:    "completed",
			Comment:   result.Output,
			SessionID: result.SessionID,
			WorkDir:   env.WorkDir,
...
func (d *Daemon) executeAndDrain(ctx context.Context, backend agent.Backend, prompt string, opts agent.ExecOptions, taskLog *slog.Logger, taskID, codexHome string, msgSeq *atomic.Int32, transcriptDelivered *atomic.Bool) (agent.Result, int32, error) {
