...
type PrepareParams struct {
	...
	LocalWorkDir string
	// Isolate selects the fork's task-scoped worktree for resources that use
	// publish_back. It is mutually exclusive with the upstream conversation
	// worktree selected by LocalWorktree.
	Isolate     bool
	PublishBack bool
...
func Prepare(params PrepareParams, logger *slog.Logger) (*Environment, error) {
	...
	if isolate && params.LocalWorktree != nil {
		return nil, fmt.Errorf("execenv: task-scoped and conversation worktrees are mutually exclusive")
	}
	if params.LocalWorkDir == "" && params.LocalWorktree == nil {
		scratchDirs = append(scratchDirs, workDir)
	} else if params.LocalWorkDir != "" && !isolate {
