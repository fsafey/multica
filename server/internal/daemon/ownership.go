...
func (e *OwnershipConflict) Error() string {
	...
	return fmt.Sprintf(
		"another Multica daemon already owns this machine via lock %s (owner details unavailable); stop it with `multica daemon stop` or take over with `multica daemon start --takeover`",
		e.Path,
	)
}
