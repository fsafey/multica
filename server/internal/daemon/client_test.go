...
func TestFailTask_RetriesOnTransient5xxThenSucceeds(t *testing.T) {
	...
	c := NewClient(srv.URL)
	if err := c.FailTask(context.Background(), "task-1", "boom", "", "", "timeout", TranscriptExpectation{}, true, ""); err != nil {
	...
	for _, tc := range []struct {
	...
	}{
		{
			...
			call: func(c *Client) error {
				return c.CompleteTask(context.Background(), "task-1", "done", "", "", "/tmp/wd", TranscriptExpectation{}, false, "POISONED-S")
			},
		},
		{
			name:     "fail",
			...
			call: func(c *Client) error {
				return c.FailTask(context.Background(), "task-1", "boom", "", "/tmp/wd", "api_invalid_request", TranscriptExpectation{}, false, "POISONED-S")
			},
		},
	} {
		t.Run(tc.name, func(t *testing.T) {
...
func TestTerminalReportsOmitEmptyRetiredSessionID(t *testing.T) {
	...
	if err := NewClient(srv.URL).CompleteTask(context.Background(), "task-1", "done", "", "sess-1", "/tmp/wd", TranscriptExpectation{}, false, ""); err != nil {
