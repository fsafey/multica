...
VALUES ($1, 'Workspace delete workflow project')
RETURNING id
`, wsID).Scan(&workflowProjectID); err != nil {
		t.Fatalf("create workflow project: %v", err)
	}
	var runtimePoolID string
	if err := testPool.QueryRow(ctx, `
INSERT INTO runtime_pool (workspace_id, name, created_by)
VALUES ($1, 'workspace-delete-pool', $2)
RETURNING id
`, wsID, testUserID).Scan(&runtimePoolID); err != nil {
		t.Fatalf("create workflow runtime pool: %v", err)
	}
	if _, err := testPool.Exec(ctx, `
INSERT INTO runtime_pool_runtime (pool_id, runtime_id)
VALUES ($1, $2)
`, runtimePoolID, runtimeID); err != nil {
		t.Fatalf("bind runtime to workflow pool: %v", err)
	}
	if _, err := testPool.Exec(ctx, `
INSERT INTO agent_runtime_pool (agent_id, pool_id)
VALUES ($1, $2)
`, agentID, runtimePoolID); err != nil {
		t.Fatalf("bind agent to workflow pool: %v", err)
	}
	var workflowRunID string
	if err := testPool.QueryRow(ctx, `
INSERT INTO workflow_run (
	workspace_id, project_id, anchor_issue_id, graph_key, graph_version,
	integration_pool_id, created_by
)
VALUES ($1, $2, $3, 'workspace-delete', 'v1', $4, $5)
RETURNING id
`, wsID, workflowProjectID, issueID, runtimePoolID, testUserID).Scan(&workflowRunID); err != nil {
		t.Fatalf("create workflow run: %v", err)
	}
	var workflowNodeID, dependencyNodeID string
	if err := testPool.QueryRow(ctx, `
INSERT INTO workflow_node (
	run_id, issue_id, passage_key, node_key, executor_kind, agent_id, runtime_pool_id
)
VALUES ($1, $2, 'passage-1', 'translate', 'agent', $3, $4)
RETURNING id
`, workflowRunID, issueID, agentID, runtimePoolID).Scan(&workflowNodeID); err != nil {
		t.Fatalf("create workflow node: %v", err)
	}
	if err := testPool.QueryRow(ctx, `
INSERT INTO workflow_node (run_id, issue_id, passage_key, node_key, executor_kind)
VALUES ($1, $2, 'passage-1', 'review', 'human_gate')
RETURNING id
`, workflowRunID, issueID).Scan(&dependencyNodeID); err != nil {
		t.Fatalf("create dependent workflow node: %v", err)
	}
	if _, err := testPool.Exec(ctx, `
INSERT INTO workflow_node_dependency (node_id, depends_on_node_id)
VALUES ($1, $2)
`, dependencyNodeID, workflowNodeID); err != nil {
		t.Fatalf("create workflow dependency: %v", err)
	}
	var workflowAttemptID string
	if err := testPool.QueryRow(ctx, `
INSERT INTO workflow_node_attempt (
	node_id, claim_epoch, task_id, runtime_id, daemon_id, lease_expires_at
)
VALUES ($1, 1, $2, $3, 'workspace-delete-daemon', now() + interval '1 minute')
RETURNING id
`, workflowNodeID, taskID, runtimeID).Scan(&workflowAttemptID); err != nil {
		t.Fatalf("create workflow attempt: %v", err)
	}
	if _, err := testPool.Exec(ctx, `
INSERT INTO workflow_node_result (
	node_id, attempt_id, generation, claim_epoch, canonical_commit, artifact_digest
)
VALUES ($1, $2, 1, 1, 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa', 'sha256:workflow-delete')
`, workflowNodeID, workflowAttemptID); err != nil {
		t.Fatalf("create workflow result: %v", err)
	}
	if _, err := testPool.Exec(ctx, `
INSERT INTO workflow_node_resource (node_id, resource_key)
VALUES ($1, 'workspace-delete-resource')
`, workflowNodeID); err != nil {
		t.Fatalf("create workflow node resource: %v", err)
	}
	if _, err := testPool.Exec(ctx, `
INSERT INTO workflow_resource_claim (resource_key, node_id, attempt_id)
VALUES ('workspace-delete-resource', $1, $2)
`, workflowNodeID, workflowAttemptID); err != nil {
		t.Fatalf("create workflow resource claim: %v", err)
	}
	var workflowOutboxID string
	if err := testPool.QueryRow(ctx, `
INSERT INTO workflow_outbox (run_id, node_id, event_type)
VALUES ($1, $2, 'workspace.delete.test')
RETURNING id
`, workflowRunID, workflowNodeID).Scan(&workflowOutboxID); err != nil {
		t.Fatalf("create workflow outbox event: %v", err)
	}
...
`, pendingObjectKey, wsID); err != nil {
	...
	t.Cleanup(func() {
		_, _ = testPool.Exec(context.Background(), `DELETE FROM workflow_resource_claim WHERE node_id IN ($1, $2)`, workflowNodeID, dependencyNodeID)
		_, _ = testPool.Exec(context.Background(), `DELETE FROM workflow_node_result WHERE node_id IN ($1, $2)`, workflowNodeID, dependencyNodeID)
		_, _ = testPool.Exec(context.Background(), `DELETE FROM workflow_node_attempt WHERE node_id IN ($1, $2)`, workflowNodeID, dependencyNodeID)
		_, _ = testPool.Exec(context.Background(), `DELETE FROM workflow_node_resource WHERE node_id IN ($1, $2)`, workflowNodeID, dependencyNodeID)
		_, _ = testPool.Exec(context.Background(), `DELETE FROM workflow_node_dependency WHERE node_id IN ($1, $2) OR depends_on_node_id IN ($1, $2)`, workflowNodeID, dependencyNodeID)
		_, _ = testPool.Exec(context.Background(), `DELETE FROM workflow_outbox WHERE id = $1`, workflowOutboxID)
		_, _ = testPool.Exec(context.Background(), `DELETE FROM workflow_node WHERE id IN ($1, $2)`, workflowNodeID, dependencyNodeID)
		_, _ = testPool.Exec(context.Background(), `DELETE FROM workflow_run WHERE id = $1`, workflowRunID)
		_, _ = testPool.Exec(context.Background(), `DELETE FROM agent_runtime_pool WHERE pool_id = $1`, runtimePoolID)
		_, _ = testPool.Exec(context.Background(), `DELETE FROM runtime_pool_runtime WHERE pool_id = $1`, runtimePoolID)
		_, _ = testPool.Exec(context.Background(), `DELETE FROM runtime_pool WHERE id = $1`, runtimePoolID)
	...
	workflowChecks := map[string]struct {
		column string
		id     string
	}{
		"agent_runtime_pool":       {column: "pool_id", id: runtimePoolID},
		"runtime_pool":             {column: "id", id: runtimePoolID},
		"runtime_pool_runtime":     {column: "pool_id", id: runtimePoolID},
		"workflow_node":            {column: "id", id: dependencyNodeID},
		"workflow_node_attempt":    {column: "id", id: workflowAttemptID},
		"workflow_node_dependency": {column: "node_id", id: dependencyNodeID},
		"workflow_node_resource":   {column: "node_id", id: workflowNodeID},
		"workflow_node_result":     {column: "node_id", id: workflowNodeID},
		"workflow_outbox":          {column: "id", id: workflowOutboxID},
		"workflow_resource_claim":  {column: "node_id", id: workflowNodeID},
		"workflow_run":             {column: "id", id: workflowRunID},
	}
	for table, check := range workflowChecks {
		var count int
		if err := testPool.QueryRow(ctx, `SELECT COUNT(*) FROM `+table+` WHERE `+check.column+` = $1`, check.id).Scan(&count); err != nil {
			t.Fatalf("verify %s cleanup: %v", table, err)
		}
		if count != 0 {
			t.Fatalf("%s row survived workspace delete", table)
		}
	}
