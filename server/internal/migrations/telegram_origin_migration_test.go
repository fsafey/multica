...
func TestTelegramOriginMigrationsUpDownAndCatalog(t *testing.T) {
	...
	applyMigrationFile(t, ctx, conn.Conn(), "402_issue_origin_telegram_chat.up.sql")
	...
	applyMigrationFile(t, ctx, conn.Conn(), "403_issue_origin_telegram_chat_validate.up.sql")
	assertTelegramOriginConstraint(t, ctx, conn.Conn(), true, true)

	applyMigrationFile(t, ctx, conn.Conn(), "403_issue_origin_telegram_chat_validate.down.sql")
	assertTelegramOriginConstraint(t, ctx, conn.Conn(), false, true)
	...
	applyMigrationFile(t, ctx, conn.Conn(), "402_issue_origin_telegram_chat.down.sql")
