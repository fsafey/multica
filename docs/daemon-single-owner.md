...
graceful shutdown over HTTP (cross-platform; no OS signals), waits up to 30s for
the lock to free, then acquires it. If the incumbent does not release in time it
fails loudly rather than forcing.
